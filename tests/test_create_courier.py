...
import allure
import requests
from helpers import login_courier, delete_courier, generate_random_string
...
class TestCreateCourier:
    ...
    def test_create_courier_returns_ok_true(self, courier_payload):
        with allure.step('Отправляем запрос'):
            response = requests.post(CREATE_COURIER_URL, json=courier_payload)
    ...
    def test_cannot_create_duplicate_courier(self, courier_payload, cleanup):
    ...
    def test_duplicate_courier_returns_error_message(self, courier_payload, cleanup):
    ...
    def test_create_courier_without_login_returns_400(self, cleanup):
        payload = {
            "password": generate_random_string(10),
            "firstName": generate_random_string(10)
        }
    ...
    def test_create_courier_without_password_returns_400(self, cleanup):
        payload = {
            "login": generate_random_string(10),
            "firstName": generate_random_string(10)
        }
